...
const AuthForm = ({ view, setView }: { view: 'login' | 'register', setView: (v: 'landing' | 'login' | 'register') => void }) => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      console.error(err);
      if (err.message === 'privacy-rejected') {
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            ...
            disabled={loading}
            className="w-full bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-sans font-bold py-4 rounded-xl transition-all transform active:scale-[0.98] disabled:opacity-70 shadow-sm flex items-center justify-center gap-3"
          >
            ...
            {view === 'login' ? 'Entrar amb Google' : 'Continuar amb Google'}
          </button>
        </form>
